...
func _physics_process(delta: float) -> void:
	...
	if is_sliding:
		...
		if cancel_slide:
			...
			# Lướt trên không nối sang jump attack; chỉ cú lướt dưới đất mới dùng
			# animation và vùng chém SlideShape riêng.
			if Input.is_action_just_pressed("attack") and can_start_attack():
				if not is_on_floor():
					if try_start_jump_attack():
						stop_slide(true)
						return
				else:
...
## Bắt đầu đòn trên không cho cả trạng thái nhảy thường lẫn air slide.
func try_start_jump_attack() -> bool:
...
func stop_slide(keep_horizontal_velocity := false) -> void:
	is_sliding = false
	slide_time_left = 0.0
	if not keep_horizontal_velocity:
		velocity.x = 0.0
...
func is_jump_attack() -> bool:
	return animated_sprite.animation == JUMP_ATTACK_ANIM \
		or animated_sprite.animation == SPIN_ATTACK_ANIM
